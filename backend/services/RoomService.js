...
class RoomService {
  ...
  createFileInRoom(roomId, name, content, language) {
    const room = this.rooms.get(roomId);
    if (room) {
      return room.createFile(name, content, language);
    }
    return null;
  }

  // Update file content in room
  updateFileContent(roomId, fileId, content) {
    const room = this.rooms.get(roomId);
    if (room) {
      return room.updateFileContent(fileId, content);
    }
    return null;
  }

  // Update file language in room
  updateFileLanguage(roomId, fileId, language) {
    const room = this.rooms.get(roomId);
    if (room) {
      return room.updateFileLanguage(fileId, language);
    }
    return null;
  }

  // Rename file in room
  renameFileInRoom(roomId, fileId, newName) {
    const room = this.rooms.get(roomId);
    if (room) {
      return room.renameFile(fileId, newName);
    }
    return null;
  }

  // Delete file in room
  deleteFileInRoom(roomId, fileId) {
    const room = this.rooms.get(roomId);
    if (room) {
      return room.deleteFile(fileId);
    }
    return null;
  }

  // Set active file in room
  setActiveFileInRoom(roomId, fileId) {
    const room = this.rooms.get(roomId);
    if (room) {
      return room.setActiveFile(fileId);
    }
    return null;
  }
